...
func create_lobby():
	...
	create_opts.bucket_id = "Splash_Roulette"
	...
	var result := peer.create_server("cdsplashroulette")
	if result != OK:
...
func search_lobbies() -> bool:
	...
	var lobbies = await HLobbies.search_by_bucket_id_async("Splash_Roulette")
	if not lobbies:
	...
	var result := peer.create_client("cdsplashroulette", lobby.owner_product_user_id)
	if result != OK:
		printerr("Failed to create client: " + EOS.result_str(result))
		return false
	multiplayer.multiplayer_peer = peer
